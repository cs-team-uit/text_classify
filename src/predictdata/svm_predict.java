...
class svm_predict {
	...
	public static void svm_classify(String argv[]) throws IOException {
		...
		if (i >= argv.length - 2)
			exit_with_help();
		try {
			BufferedReader input = new BufferedReader(new FileReader(file.getAbsolutePath() + argv[i]));
			DataOutputStream output = new DataOutputStream(
					new BufferedOutputStream(new FileOutputStream(file.getAbsolutePath() + argv[i + 2])));
			svm_model model = svm.svm_load_model(file.getAbsolutePath() + argv[i + 1]);
			if (model == null) {
				System.err.print("can't open model file " + file.getAbsolutePath() + argv[i + 1] + "\n");
				System.exit(1);
			}
			if (predict_probability == 1) {
